...
class NoisyImageGenerator(Sequence):
    def __init__(
        self, image_dir, source_noise_model, target_noise_model, batch_size=32, image_size=64
    ):
        self.image_paths = get_image_file_paths(image_dir)
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            image = cv2.imread(str(image_path))  # Warning: BGR, not RGB
            h, w, _ = image.shape
...
class ValGenerator(Sequence):
    def __init__(self, image_dir, val_noise_model):
        ...
        image_paths = [
            p for p in Path(image_dir).glob("**/*") if p.suffix.lower() in image_suffixes
        ]
        ...
        for image_path in image_paths:
            y = cv2.imread(str(image_path))  # Warning: BGR, not RGB
            h, w, _ = y.shape
